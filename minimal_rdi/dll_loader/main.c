...
int main(void) {
    ...
    /*解析 DLL 文件的 PE 头以提取重要信息*/
    PIMAGE_DOS_HEADER dos_headers = (PIMAGE_DOS_HEADER) dll_bytes;
    PIMAGE_NT_HEADERS ntHeaders = (PIMAGE_NT_HEADERS) ((DWORD_PTR) dll_bytes + dos_headers->e_lfanew);
    SIZE_T dllImageSize = ntHeaders->OptionalHeader.SizeOfImage;
    /*在目标进程的地址空间中分配内存以保存二进制文件*/
    LPVOID dll_base = VirtualAlloc((LPVOID) ntHeaders->OptionalHeader.ImageBase, dllImageSize,
    ...
    while (relocationsProcessed < relocations.Size) {
        ...
        for (DWORD i = 0; i < relocationsCount; i++) {
            ...
            ReadProcessMemory(GetCurrentProcess(), (LPCVOID) ((DWORD_PTR) dll_base + relocationRVA), &addressToPatch,
